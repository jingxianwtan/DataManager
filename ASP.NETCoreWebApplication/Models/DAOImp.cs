...
namespace ConsoleApp1
{
    public class DAOImp : DataAccessObject
    {
        ...
        public IEnumerable<Event> GetEventsByCategory(string catId)
        {
            List<Event> result = new List<Event>();
            try
            {
                conn.Open();
                string sql = string.Format("SELECT * FROM event WHERE category_id = {0}", catId);
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                MySqlDataReader dr = cmd.ExecuteReader();
        ...
        public IEnumerable<Listing> GetListingsOfEvent(string eventId)
        {
            List<Listing> result = new List<Listing>();
            try
            {
                conn.Open();
                string sql = string.Format("SELECT * FROM listing WHERE event_id = {0}", eventId);
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                MySqlDataReader dr = cmd.ExecuteReader();
        ...
        public IEnumerable<Node> GetACategoriesByPid(string pid)
        {
            List<Node> result = new List<Node>();
            try
            {
                conn.Open();
                string sql = string.Format("SELECT * FROM category WHERE parent_cat_id = {0}", pid);
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                MySqlDataReader dr = cmd.ExecuteReader();
        ...
        public List<Transactions> GetTransactionsByListing(string lid)
        {
            List<Transactions> result = new List<Transactions>();
            try
            {
                conn.Open();
                string sql = string.Format("SELECT * FROM transaction WHERE listing_id = {0}", lid);
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                MySqlDataReader dr = cmd.ExecuteReader();
